...
def inner_rk(run_time, state, f_max, dt, rko):
    ...
    def make_partial(c, post, pre):
        if isinstance(c, sparse.csr_matrix):
...
def make_state():
    ...
    for post, all_pre in state['conn'].items():
        for pre in all_pre:
            ...
            # connectivity matrix initialisation
            my_c = np.zeros((cd['N_pre'], cd['N_post']))
            # potential connections
            my_ps = np.random.uniform(size=cd['N_pre']*cd['N_post']).reshape((cd['N_pre'], cd['N_post']))
            my_probability_of_connectivity = 1.1
            # save connections which have chosen probability
            my_c[my_ps < my_probability_of_connectivity] = cd['J']
            print(cd['J'])
            my_c = np.transpose(my_c)
            # save to state dictionary
            all_pre[pre] = my_c
            #all_pre[pre] = # insert random matrix here # compute_connections(**cd)[0]
    if single_array:
